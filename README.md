...
1. `miditoyz query` - query your song catalog to create a playlist you can use to stream particular songs. The query command uses the [SQL "where" clause](https://www.google.com/search?q=sql+where+clause+introduction+tutorial) syntax, providing lots of flexibility in terms of song selection, sorting, etc.. You can see the structure of your catalog using the `miditoyz fields` command.
